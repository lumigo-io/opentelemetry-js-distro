    ...
    describe('with Task metadata endpoint version 4', () => {
        ...
        it('detects resource attributes correctly', async () => {
            jest.spyOn(AwsEcsDetector.prototype, 'getUri').mockImplementation((url: string) => {
                ...
                switch (url) {
                    case 'test_url':
                        responseRaw = fs.readFileSync(__dirname + '/test-resources/metadatav4-response-container.json');
                        break;
                    case 'test_url/task': 
                        responseRaw = fs.readFileSync(__dirname + '/test-resources/metadatav4-response-task.json');
                        break;
                    default:
